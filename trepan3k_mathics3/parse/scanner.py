...
Copyright (c) 2017-2018, 2026 Rocky Bernstein
"""
...
from trepan3k_mathics3.parse.tok import Token
...
class LocationScanner(GenericScanner):
    ...
    # A Mathics3 function like Refine[]
    # In the future we may ad operator symbols
    def t_mathics3_func(self, s):
        r'(?:[A-Z][^\'"\t \n:,]*)(?:\[\])?'
        m = s[:-2] if s.endswith("[]") else s
        self.add_token("FUNCNAME", m)
        self.pos += len(s)
